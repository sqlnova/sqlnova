...
export const LECCIONES_M9: Leccion[] = [
  ...
  {
    id: '09-05', num: 5, titulo: 'Cuando NO usar indices', tipo: 'completar',
    dificultad: 'intermedio', xp: 20, tabla: 'usuarios',
    teoria: 'Los indices <strong>no siempre</strong> mejoran el rendimiento. No convienen en: columnas con muy pocos valores distintos (un booleano activo/inactivo no ayuda si la mitad de filas tienen el mismo valor), tablas pequenas (el full scan es mas rapido), o columnas que se modifican muy frecuentemente (el indice se reconstruye en cada UPDATE).',
    enunciado: 'La tabla usuarios tiene columnas: id, nombre, email, pais, es_premium. Cual tiene <strong>menos sentido indexar</strong> para optimizar busquedas? Completa: La columna menos util para indexar es ___',
    pista: 'es_premium — solo tiene dos valores posibles (true/false), por lo que el indice no ayuda a filtrar significativamente',
    solucion: 'es_premium',
    template: 'La columna menos util para indexar es ___',
    blanks: ['es_premium'],
  },
  {
  ...
  },
  {
    ...
    enunciado: 'Arma un query de paginacion para la <strong>tercera pagina</strong> del feed, mostrando <strong>20 posts por pagina</strong>. Mostra id, titulo y fecha, ordenados por fecha DESC.',
    pista: 'SELECT id, titulo, fecha FROM posts ORDER BY fecha DESC LIMIT 20 OFFSET 40',
    solucion: 'SELECT id, titulo, fecha FROM posts ORDER BY fecha DESC LIMIT 20 OFFSET 40',
  },
  {
    ...
    enunciado: 'Este query es muy lento en produccion. Identifica el problema y reescribilo de forma optimizada:\n\n<code>SELECT * FROM posts WHERE LOWER(titulo) = \'hola mundo\'</code>',
    pista: "SELECT id, titulo, fecha, cantidad_likes FROM posts WHERE titulo = 'Hola Mundo'",
    solucion: "SELECT id, titulo, fecha, cantidad_likes FROM posts WHERE titulo = 'Hola Mundo'",
    errorQuery: "SELECT * FROM posts WHERE LOWER(titulo) = 'hola mundo'",
  },
  {
...
export const LECCIONES_M10: Leccion[] = [
  {
    ...
    teoria: 'Una pregunta clasica de entrevistas: encontrar el N-esimo valor mas alto. La solucion mas limpia usa una subquery para excluir el maximo y luego toma el maximo del resto. Alternativa: <code>SELECT DISTINCT salario FROM empleados ORDER BY salario DESC LIMIT 1 OFFSET 1</code>.',
    enunciado: 'Pregunta de entrevista clasica: encontra el <strong>segundo salario mas alto</strong> de la tabla empleados. Mostra solo el valor del salario, sin el nombre.',
  ...
  },
  {
    ...
    enunciado: 'Encontra los usuarios que <strong>NO publicaron ningun post en los ultimos 30 dias</strong>. La tabla posts tiene una columna fecha. Mostra id y nombre de esos usuarios.',
    pista: "SELECT u.id, u.nombre FROM usuarios u LEFT JOIN posts p ON u.id = p.usuario_id AND p.fecha >= date('now', '-30 days') WHERE p.id IS NULL",
    solucion: "SELECT u.id, u.nombre FROM usuarios u LEFT JOIN posts p ON u.id = p.usuario_id AND p.fecha >= date('now', '-30 days') WHERE p.id IS NULL",
  },
  {
  ...
  },
  {
    ...
    enunciado: 'Calcula el <strong>total acumulado de ventas aprobadas</strong> por fecha, ignorando las rechazadas. De la tabla <strong>ventas</strong> mostra: fecha, monto, estado y el acumulado solo de las aprobadas (llamalo <strong>acumulado_aprobadas</strong>). Ordena por fecha.',
    pista: "SELECT fecha, monto, estado, SUM(CASE WHEN estado = 'aprobada' THEN monto ELSE 0 END) OVER(ORDER BY fecha) AS acumulado_aprobadas FROM ventas ORDER BY fecha",
    solucion: "SELECT fecha, monto, estado, SUM(CASE WHEN estado = 'aprobada' THEN monto ELSE 0 END) OVER(ORDER BY fecha) AS acumulado_aprobadas FROM ventas ORDER BY fecha",
  },
  {
  ...
  },
  {
    ...
    enunciado: 'Arma el reporte mensual. Por usuario mostra: <strong>nombre</strong>, <strong>pais</strong>, <strong>total_posts</strong> del mes actual, <strong>total_likes</strong>, <strong>ranking_pais</strong> por likes dentro del pais (RANK), y <strong>categoria</strong> como "Influencer" si supera 1000 likes o "Regular" si no. Solo usuarios con al menos 1 post. Ordena por pais y ranking_pais.',
    pista: "WITH stats AS (SELECT u.id, u.nombre, u.pais, COUNT(p.id) AS total_posts, SUM(p.cantidad_likes) AS total_likes FROM usuarios u INNER JOIN posts p ON u.id = p.usuario_id WHERE strftime('%Y-%m', p.fecha) = strftime('%Y-%m', 'now') GROUP BY u.id, u.nombre, u.pais HAVING COUNT(p.id) >= 1) SELECT nombre, pais, total_posts, total_likes, RANK() OVER(PARTITION BY pais ORDER BY total_likes DESC) AS ranking_pais, CASE WHEN total_likes > 1000 THEN 'Influencer' ELSE 'Regular' END AS categoria FROM stats ORDER BY pais, ranking_pais",
